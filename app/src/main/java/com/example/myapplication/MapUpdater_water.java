...
public class MapUpdater_water {
    ...
    public static void updateImageView(Context context, int drawableId, ImageView imageView, String selectedITem) {
        ...
        for (int i = 0; i < regionNames.length; i++) {
            ...
            water_sum += (int) temperatures;
        }

        int water_mean = water_sum / regionNames.length;
        System.out.println(water_mean);
        for (int i = 0; i < tmp.length; i++) {
            if (water_mean < tmp[i]) {
                int value = tmp[i] * 7;
                tmp[i] = (value > 255) ? 255 : value;
            } else {
                int value = ((water_mean - tmp[i]) + 1) * 100;
                tmp[i] = value;
            }
        }

        for (int i = 0; i < tmp.length; i++) {
            int color = Color.argb(tmp[i], 0, 0, 255);
